...
namespace PROG_POE_PART_2_AGRI_ENEGRY.Areas.Identity.Pages.Account
{
    ...
    {
        private readonly SignInManager<PlatformUsers> _signInManager;
        private readonly UserManager<PlatformUsers> _userManager;
        private readonly IUserStore<PlatformUsers> _userStore;
        ...
        public RegisterModel(
            ...
            IUserStore<PlatformUsers> userStore,
        ...
        private PlatformUsers CreateUser()
        {
            try
            {
                return Activator.CreateInstance<PlatformUsers>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(PlatformUsers)}'. " +
                    $"Ensure that '{nameof(PlatformUsers)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<PlatformUsers> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<PlatformUsers>)_userStore;
        }
    }
}
